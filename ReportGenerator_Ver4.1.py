...
while True:
    ...
    for executeCount in range(len(limitFilePath)):
        ...
        if True:
            ...
            if name in ExcelWS:
                ...
                if normal != "":
                    ...
                    NormalWSamount = NormalWS.max_row
                else:
                    NormalWSamount = 0
                if low != "":
                ...
                if high != "":
                    ...
                    HighWSamount = HighWS.max_row
                else:
                    HighWSamount = 0
                if editWS.max_row != NormalWSamount + LowWSamount + HighWSamount + 9:
                    print("The existing Report is different from the Limit File, please run Limit File Update Version\n")
                    pause()
                for editWSname in range(10, editWS.max_row+1):
                    if editWS.cell(row=editWSname, column=4).value == "" or editWS.cell(row=editWSname, column=4).value == None:
                        print("The existing Report is different from the Limit File, please run Limit File Update Version\n")
                        pause()
                                    ...
                                    if trueCounter <= count*4 and trueCounter >= compare:
                                        ...
                                        for groupingCount in range(count):
                                            ...
                                            if TestDataWS.cell(row=TestDataWsCount+groupingCount, column=26).value != "" and nCoordinate != "non":
                                                ...
                                                if (TestDataWS.cell(row=TestDataWsCount+groupingCount, column=26).value) is not None:
                                                    splitR = (TestDataWS.cell(row=TestDataWsCount+groupingCount, column=26).value).split(".")
                                                    splitRev = ""
                                                    for i in range(len(splitR)):
                                                        splitRev = splitRev + splitR[i]
                                                    reRev.append(int(splitRev))
                                                    coRev.append(int(splitRev))
                                            if validCoordinate != "novalid":
        ...
        if True:
            ...
            """
            revision = []
            for i in range(resultWS.max_row):
                if resultWS.cell(row=i+1, column=2).value == name:
                    revision.append(resultWS.cell(row=i, column=7).value)
            for i in range(len(revision)):
                if "," in revision[i]:
                    splitOverviewR = ((revision[i]).split(", "))
                    splitOverviewR0 = ((splitOverviewR[0]).split("."))
                    splitOverviewR1 = ((splitOverviewR[1]).split("."))
                    splitOverviewRev0 = ""
                    splitOverviewRev1 = ""
                    for i in range(splitOverviewR0):
                        splitOverviewRev0 = splitOverviewRev0 + splitOverviewR0[i]
                    for i in range(splitOverviewR1):
                        splitOverviewRev1 = splitOverviewRev1 + splitOverviewR1[i]
                    reRev.append(int(splitOverviewRev0))
                    reRev.append(int(splitOverviewRev1))
                else:
                    splitOverviewR = ((revision[i]).split("."))
                    splitOverviewRev = ""
                    for i in range(splitOverviewR):
                        splitOverviewRev = splitOverviewRev + splitOverviewR[i]
                    reRev.append(int(splitOverviewRev))
            reRev.sort()
            resultRev = ""
            if reRev[0] == reRev[-1]:
                reRev[-1] = str(reRev[-1])
                if len(reRev[-1]) == 5:
                    resultreRev =                   (reRev[-1])[-5] + "." + (reRev[-1])[-4] + (reRev[-1])[-3] + "." + (reRev[-1])[-2] + (reRev[-1])[-1]
                if len(reRev[-1]) == 6:
                    resultreRev = (reRev[-1])[-6] + (reRev[-1])[-5] + "." + (reRev[-1])[-4] + (reRev[-1])[-3] + "." + (reRev[-1])[-2] + (reRev[-1])[-1]
            else:
                reRev[-1] = str(reRev[-1])
                reRev[0] = str(reRev[0])
                if len(reRev[-1]) == 5:
                    resultRev1 =                   (reRev[-1])[-5] + "." + (reRev[-1])[-4] + (reRev[-1])[-3] + "." + (reRev[-1])[-2] + (reRev[-1])[-1]
                if len(reRev[-1]) == 6:
                    resultRev1 = (reRev[-1])[-6] + (reRev[-1])[-5] + "." + (reRev[-1])[-4] + (reRev[-1])[-3] + "." + (reRev[-1])[-2] + (reRev[-1])[-1]
                if len(reRev[0]) == 5:
                    resultRev0 =                  (reRev[0])[-5] + "." + (reRev[0])[-4] + (reRev[0])[-3] + "." + (reRev[0])[-2] + (reRev[0])[-1]
                if len(reRev[0]) == 6:
                    resultRev0 = (reRev[0])[-6] + (reRev[0])[-5] + "." + (reRev[0])[-4] + (reRev[0])[-3] + "." + (reRev[0])[-2] + (reRev[0])[-1]
                resultRev = resultRev1 + ", " + resultRev0
            """
            ...
            #resultWS.cell(row=resultWS.max_row, column=7).value = resultRev
            ...
            """
            revision = []
            for i in range(coverageWS.max_row):
                if coverageWS.cell(row=i+1, column=2).value == name:
                    revision.append(coverageWS.cell(row=i, column=7).value)
            for i in range(len(revision)):
                if "," in revision[i]:
                    splitOverviewR = ((revision[i]).split(", "))
                    splitOverviewR0 = ((splitOverviewR[0]).split("."))
                    ...
                    for i in range(splitOverviewR1):
                        splitOverviewRev1 = splitOverviewRev1 + splitOverviewR1[i]
                    coRev.append(int(splitOverviewRev0))
                    coRev.append(int(splitOverviewRev1))
                else:
                    splitOverviewR = ((revision[i]).split("."))
                    splitOverviewRev = ""
                    for i in range(splitOverviewR):
                        splitOverviewRev = splitOverviewRev + splitOverviewR[i]
                    coRev.append(int(splitOverviewRev))
            coRev.sort()
            if coRev[0] == coRev[-1]:
                coRev[-1] = str(coRev[-1])
                if len(coRev[-1]) == 5:
                    coverageRev =                   (coRev[-1])[-5] + "." + (coRev[-1])[-4] + (coRev[-1])[-3] + "." + (coRev[-1])[-2] + (coRev[-1])[-1]
                if len(coRev[-1]) == 6:
                    coverageRev = (coRev[-1])[-6] + (coRev[-1])[-5] + "." + (coRev[-1])[-4] + (coRev[-1])[-3] + "." + (coRev[-1])[-2] + (coRev[-1])[-1]
            else:
                coRev[-1] = str(coRev[-1])
                coRev[0] = str(coRev[0])
                if len(coRev[-1]) == 5:
                    coverageRev1 =                   (coRev[-1])[-5] + "." + (coRev[-1])[-4] + (coRev[-1])[-3] + "." + (coRev[-1])[-2] + (coRev[-1])[-1]
                if len(coRev[-1]) == 6:
                    coverageRev1 = (coRev[-1])[-6] + (coRev[-1])[-5] + "." + (coRev[-1])[-4] + (coRev[-1])[-3] + "." + (coRev[-1])[-2] + (coRev[-1])[-1]
                if len(coRev[0]) == 5:
                    coverageRev0 =                  (coRev[0])[-5] + "." + (coRev[0])[-4] + (coRev[0])[-3] + "." + (coRev[0])[-2] + (coRev[0])[-1]
                if len(coRev[0]) == 6:
                    coverageRev0 = (coRev[0])[-6] + (coRev[0])[-5] + "." + (coRev[0])[-4] + (coRev[0])[-3] + "." + (coRev[0])[-2] + (coRev[0])[-1]
                coverageRev = coverageRev1 + ", " + coverageRev0
            """
            ...
            #coverageWS.cell(row=coverageWS.max_row, column=7).value = coverageRev
