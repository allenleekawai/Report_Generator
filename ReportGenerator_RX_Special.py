...
while True:
    ...
    for executeCount in range(len(limitFilePath)):
        ...
        if True:
            ...
            if any("RX_OOB" in s for s in LimitFileWS) or any("RX_OBB" in s for s in LimitFileWS) or any("RX_COL" in s for s in LimitFileWS):

                if name in ExcelWS:
                    editWS = ExcelWB[name]
                else:
                    print("Building test item by the limit file\n\n========================================\n")
                    Example = ExcelWB["SpcExample"]
                    ...
                    if normal != "":
                        ...
                        for NormalWsCount in range(1, NormalWS.max_row+1):
                            ...
                            editWS.cell(row=NormalWsCount+9, column=9).value = Unit
                            editWS.cell(row=NormalWsCount+9, column=10).value = Compare
                            editWS.cell(row=NormalWsCount+9, column=11).value = L_LMT
                            editWS.cell(row=NormalWsCount+9, column=12).value = H_LMT
                    ...
                    if low != "":
                        ...
                        for LowWsCount in range(1, LowWS.max_row+1):
                            ...
                            editWS.cell(row=Counter+LowWsCount+9, column=9).value = Unit
                            editWS.cell(row=Counter+LowWsCount+9, column=10).value = Compare
                            editWS.cell(row=Counter+LowWsCount+9, column=11).value = L_LMT
                            editWS.cell(row=Counter+LowWsCount+9, column=12).value = H_LMT
                    ...
                    if high != "":
                        ...
                        for HighWsCount in range(1, HighWS.max_row+1):
                            ...
                            editWS.cell(row=Counter+HighWsCount+9, column=9).value = Unit
                            editWS.cell(row=Counter+HighWsCount+9, column=10).value = Compare
                            editWS.cell(row=Counter+HighWsCount+9, column=11).value = L_LMT
                            editWS.cell(row=Counter+HighWsCount+9, column=12).value = H_LMT

                            HighCount = HighWsCount

                        Counter = Counter + HighCount
            else:
                print("This version is only for RX_OOB, RX_OBB and RX_COL")
                print("If you are generating other test item, please change the tool")
                pause()
        ...
        if True:
            ...
            for row in editWS['A9':'AZ9']:
                for cell in row:
                    ...
                    if cell.value == "Ref":
                        refCoordinate = cell.column
                    if cell.value == "Intf":
                        intfCoordinate = cell.column
                    if cell.value == "Delta":
                        valueCoordinate = cell.column
                    if cell.value == "Ref_Image_Local":
                        refimageLocalCoordinate = cell.column
                    if cell.value == "Intf_Image_Local":
                        intfimageLocalCoordinate = cell.column
                    if cell.value == "Ref_Image_Server":
                        refimageServerCoordinate = cell.column
                    if cell.value == "Intf_Image_Server":
                        intfimageServerCoordinate = cell.column
            ...
            refpicName = ["Name"] * (editWS.max_row + 1 - 10)
            testPlot = ["Plot"] * (editWS.max_row + 1 - 10)
            reftestPlot = ["Plot"] * (editWS.max_row + 1 - 10)
        ...
        def TestDataBuilding(rowCount, count):
            ...
            global temp0, picDir1, picDir2, twodarray, execTime, ftpFolderLink, newFTPFolderTestItemUserPath
            temp0 = rowCount-9
            picDir1 = []
            picDir2 = []
            ...
            if (None in result) or ("Failed" in result) or ("Skipped" in result) or ("Invalid" in valid):
                        ...
                        for testDataDirCount in range(len(testDataDir)):
                            if testDataDir[testDataDirCount][-16:] == "_TestReport.xlsx" and testDataDir[testDataDirCount][:1] == "(" and testDataDir[testDataDirCount][:2] != "~$":
                                ...
                                for TestDataWsCount in range(12, TestDataWS.max_row+1):
                                    ...
                                    trueCounter = 0
                                    for groupingCount in range(count):
                                        if editWS.cell(row=rowCount+groupingCount, column=10).value == TestDataWS.cell(row=TestDataWsCount+groupingCount, column=7).value:
                                            ...
                                            if TestDataWS.cell(row=TestDataWsCount+groupingCount, column=29).value != None:
                                                ...
                                                for picDir1Count in range(len(picDir1)):
                                                    if picDir1[picDir1Count] == testPlot[(rowCount+groupingCount)-10]:
                                                        ...
                                                        picName[(rowCount+groupingCount)-10] = newTestName[groupingCount] + "[NF_Intf]_" + (testDataDir[testDataDirCount][1:-1]).split(")(")[0][-4:] + ".png"
                                                        refpicName[(rowCount+groupingCount)-10] = newTestName[groupingCount] + "[NF_Ref]_" + (testDataDir[testDataDirCount][1:-1]).split(")(")[0][-4:] + ".png"
                                                    if name in picDir1[picDir1Count] and ".txt" in picDir1[picDir1Count]:
                                                        f = open(picFolder1 + "/" + picDir1[picDir1Count], 'r')
                                                        splitn = (f.read()).split("\n")
                                                        for i in range(len(splitn)):
                                                            twodarray.append(splitn[i].split("\t"))
                                                ...
                                                for picDir2Count in range(len(picDir2)):
                                                    if picDir2[picDir2Count] == testPlot[(rowCount+groupingCount)-10]:
                                                        ...
                                                        picName[(rowCount+groupingCount)-10] = newTestName[groupingCount] + "[NF_Intf]_" + (testDataDir[testDataDirCount][1:-1]).split(")(")[0][-4:] + ".png"
                                                        refpicName[(rowCount+groupingCount)-10] = newTestName[groupingCount] + "[NF_Ref]_" + (testDataDir[testDataDirCount][1:-1]).split(")(")[0][-4:] + ".png"
                                                    if name in picDir2[picDir2Count] and ".txt" in picDir2[picDir2Count]:
                                                        f = open(picFolder2 + "/" + picDir2[picDir2Count], 'r')
                                                        splitn = (f.read()).split("\n")
                                                        for i in range(len(splitn)):
                                                            twodarray.append(splitn[i].split("\t"))
                                                ...
                                                for i in range(len(twodarray)):
                                                    if editWS.cell(row=rowCount+groupingCount, column=picCoordinate).value == twodarray[i][13]:
                                                        delta = ('%.3f'%(editWS.cell(row=rowCount+groupingCount, column=valueCoordinate).value))
                                                        if twodarray[i][8] == delta:
                                                            editWS.cell(row=rowCount+groupingCount, column=refCoordinate).value = twodarray[i][4]
                                                            editWS.cell(row=rowCount+groupingCount, column=intfCoordinate).value = twodarray[i][6]
                                                            reftestPlot[rowCount+groupingCount-10] = twodarray[i][11] + ".png"
                ...
                for groupingCount in range(count):
                    if picPath[rowCount+groupingCount-10] != "Path" and picName[rowCount+groupingCount-10] != "Name":
                        ...
                        if reftestPlot[rowCount+groupingCount-10] == reftestPlot[rowCount+groupingCount-11] and (rowCount+groupingCount) > 10:
                            refpicName[rowCount+groupingCount-10] = refpicName[rowCount+groupingCount-11]

                        localrefnewPath = "Image/" + newImageFolder.split("/")[-1] + "/" + refpicName[rowCount+groupingCount-10]
                        serverrefnewPath = newFTPFolderTestItemUserPath + "/" + refpicName[rowCount+groupingCount-10]
                        localintfnewPath = "Image/" + newImageFolder.split("/")[-1] + "/" + picName[rowCount+groupingCount-10]
                        serverintfnewPath = newFTPFolderTestItemUserPath + "/" + picName[rowCount+groupingCount-10]
                        editWS.cell(row=rowCount+groupingCount, column=refimageLocalCoordinate).value = ('=HYPERLINK("%s", "Link")' % localrefnewPath)
                        editWS.cell(row=rowCount+groupingCount, column=refimageLocalCoordinate).font = Font(underline="single", color='00B050')
                        editWS.cell(row=rowCount+groupingCount, column=intfimageLocalCoordinate).value = ('=HYPERLINK("%s", "Link")' % localintfnewPath)
                        editWS.cell(row=rowCount+groupingCount, column=intfimageLocalCoordinate).font = Font(underline="single", color='00B050')
                        editWS.cell(row=rowCount+groupingCount, column=refimageServerCoordinate).value = ('=HYPERLINK("%s", "Link")' % serverrefnewPath)
                        editWS.cell(row=rowCount+groupingCount, column=refimageServerCoordinate).font = Font(underline="single", color='00B050')
                        editWS.cell(row=rowCount+groupingCount, column=intfimageServerCoordinate).value = ('=HYPERLINK("%s", "Link")' % serverintfnewPath)
                        editWS.cell(row=rowCount+groupingCount, column=intfimageServerCoordinate).font = Font(underline="single", color='00B050')
                        try:
                            #if picName[rowCount+groupingCount-10] not in os.listdir(newImageFolder):
                            ...
                            shutil.copyfile(os.path.join(picPath[rowCount+groupingCount-10], reftestPlot[rowCount+groupingCount-10]),os.path.join(newImageFolder, refpicName[rowCount+groupingCount-10]))
                            #pass
                        except:
