...
while True:
    ...
    if True:
        ...
        if True: # 建立 GUI
            ...
            if True: # 建立選擇 Update or New GUI

                print("Please select \"Update or New\"\n")

                # 建立使用者 GUI，使其可選擇 Config 路徑
                windowWB = tk.Tk()
                windowWB.title("Select Function")
                windowWB.geometry('200x150')
                windowWB.resizable(0,0)
                ...
                function_label = tk.Label(function_frame, text="Select Function")
                ...
                start_buttonF = tk.Button(button_frameF, text="Confirm", width=9, command=windowWB.destroy)
                start_buttonF.grid(row=0, column=0, padx=10)
                exit_buttonF = tk.Button(button_frameF, text="Close", width=9, command=exit_)
                exit_buttonF.grid(row=0, column=1, padx=10)

                windowWB.mainloop()

                print("Function select is : \n")
                if functionValue.get() == 1:
                    print("Update Data", "\n")
                if functionValue.get() == 2:
                    print("New Data", "\n")
                print("========================================\n")
    ...
    for executeCount in range(len(limitFilePath)):
        ...
        if True:
            ...
            if (name in ExcelWS) and (functionValue.get() == 2):
                print("Check if Existing Report and Limit File are same\n")
                case = 1
                editWS = ExcelWB[name]
                maxRow = editWS.max_row
                if normal != "":
                    ...
                    for NormalWsCount in range(1, NormalWS.max_row+1):
                        ...
                        editWS.cell(row=NormalWsCount+maxRow, column=1).value = TS_No
                        editWS.cell(row=NormalWsCount+maxRow, column=3).value = Temp
                        editWS.cell(row=NormalWsCount+maxRow, column=4).value = Test_Name
                        editWS.cell(row=NormalWsCount+maxRow, column=7).value = Unit
                        editWS.cell(row=NormalWsCount+maxRow, column=8).value = Compare
                        editWS.cell(row=NormalWsCount+maxRow, column=9).value = L_LMT
                        editWS.cell(row=NormalWsCount+maxRow, column=10).value = H_LMT
                ...
                if low != "":
                    ...
                    for LowWsCount in range(1, LowWS.max_row+1):
                        ...
                        editWS.cell(row=Counter+LowWsCount+maxRow, column=1).value = TS_No
                        editWS.cell(row=Counter+LowWsCount+maxRow, column=3).value = Temp
                        editWS.cell(row=Counter+LowWsCount+maxRow, column=4).value = Test_Name
                        editWS.cell(row=Counter+LowWsCount+maxRow, column=7).value = Unit
                        editWS.cell(row=Counter+LowWsCount+maxRow, column=8).value = Compare
                        editWS.cell(row=Counter+LowWsCount+maxRow, column=9).value = L_LMT
                        editWS.cell(row=Counter+LowWsCount+maxRow, column=10).value = H_LMT
                ...
                if high != "":
                    ...
                    for HighWsCount in range(1, HighWS.max_row+1):
                        ...
                        editWS.cell(row=Counter+HighWsCount+maxRow, column=1).value = TS_No
                        editWS.cell(row=Counter+HighWsCount+maxRow, column=3).value = Temp
                        editWS.cell(row=Counter+HighWsCount+maxRow, column=4).value = Test_Name
                        editWS.cell(row=Counter+HighWsCount+maxRow, column=7).value = Unit
                        editWS.cell(row=Counter+HighWsCount+maxRow, column=8).value = Compare
                        editWS.cell(row=Counter+HighWsCount+maxRow, column=9).value = L_LMT
                        editWS.cell(row=Counter+HighWsCount+maxRow, column=10).value = H_LMT

                        HighCount = HighWsCount

                    Counter = Counter + HighCount

            elif (name in ExcelWS) and (functionValue.get() == 1):
                case = 2
                editWS = ExcelWB[name]
        ...
        def TestDataBuilding(rowCount, count):
            ...
            global temp0, picDir1, picDir2, execTime, ftpFolderLink, dataUnit, newFTPFolderTestItemUserPath
            temp0 = rowCount-9
            picDir1 = []
            picDir2 = []
                        ...
                        for testDataDirCount in range(len(testDataDir)):
                            if testDataDir[testDataDirCount][-16:] == "_TestReport.xlsx" and testDataDir[testDataDirCount][:1] == "(" and testDataDir[testDataDirCount][:2] != "~$":
                                ...
                                for TestDataWsCount in range(12, TestDataWS.max_row+1):
                                    ...
                                    if functionValue.get() == 1: # 功能選擇為 Update
                                        dataUnit = (testDataDir[testDataDirCount][1:-1]).split(")(")[0]

                                        if editWS.cell(row=rowCount+groupingCount, column=unitCoordinate).value == (testDataDir[testDataDirCount][1:-1]).split(")(")[0]: # 確認兩者 Unit 相符
                                            # Grouping
                                            trueCounter = 0
                                            for groupingCount in range(count):
                                                if editWS.cell(row=rowCount+groupingCount, column=8).value == TestDataWS.cell(row=TestDataWsCount+groupingCount, column=7).value: # 確認兩者 Compare 相符，Limit File 未更改
                                            ...
                                            if trueCounter <= count*4 and trueCounter >= compare:
                                                compare = trueCounter # 計算 Passed 最多的一筆資料，作為最佳測試結果填入
                                        ...
                                        for groupingCount in range(count):
                                            if editWS.cell(row=rowCount+groupingCount, column=8).value != "RegExpr" and editWS.cell(row=rowCount+groupingCount, column=9).value != "NA":
            ...
            if functionValue.get() == 2:
                for groupingCount in range(count):
                    editWS.cell(row=rowCount+groupingCount, column=unitCoordinate).value = dataUnit
        ...
        if case == 1: # New
            for rowCounter in range(maxRow+1, editWS.max_row+1, groupCount):
                scanErrorCount = 0
                while True:
                    try:
        ...
        if True:
            ...
            resultWS.cell(row=resultWS.max_row, column=2).value = ('=IF(COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"Failed",\'%s\'!B10:\'%s\'!B%d,"%s")>0,"Failed","Passed")' % (name, name, editWS.max_row, name, name, editWS.max_row, dataUnit))
            ...
            resultWS.cell(row=resultWS.max_row, column=4).value = ('=COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"Failed",\'%s\'!C10:\'%s\'!C%d,"-40C",\'%s\'!B10:\'%s\'!B%d,"%s")&"/"&COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"",\'%s\'!C10:\'%s\'!C%d,"-40C",\'%s\'!B10:\'%s\'!B%d,"%s")+COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"Skipped",\'%s\'!C10:\'%s\'!C%d,"-40C",\'%s\'!B10:\'%s\'!B%d,"%s")' % (name, name, editWS.max_row, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit, name, name, editWS.max_row, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit, name, name, editWS.max_row, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit))
            resultWS.cell(row=resultWS.max_row, column=5).value = ('=COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"Failed",\'%s\'!C10:\'%s\'!C%d,"+25C",\'%s\'!B10:\'%s\'!B%d,"%s")&"/"&COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"",\'%s\'!C10:\'%s\'!C%d,"+25C",\'%s\'!B10:\'%s\'!B%d,"%s")+COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"Skipped",\'%s\'!C10:\'%s\'!C%d,"+25C",\'%s\'!B10:\'%s\'!B%d,"%s")' % (name, name, editWS.max_row, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit, name, name, editWS.max_row, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit, name, name, editWS.max_row, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit))
            resultWS.cell(row=resultWS.max_row, column=6).value = ('=COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"Failed",\'%s\'!C10:\'%s\'!C%d,"+55C",\'%s\'!B10:\'%s\'!B%d,"%s")&"/"&COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"",\'%s\'!C10:\'%s\'!C%d,"+55C",\'%s\'!B10:\'%s\'!B%d,"%s")+COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"Skipped",\'%s\'!C10:\'%s\'!C%d,"+55C",\'%s\'!B10:\'%s\'!B%d,"%s")' % (name, name, editWS.max_row, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit, name, name, editWS.max_row, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit, name, name, editWS.max_row, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit))
            ...
            coverageWS.cell(row=coverageWS.max_row, column=2).value = ('=IF(COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"Failed",\'%s\'!B10:\'%s\'!B%d,"%s")>0,"Failed","Passed")' % (name, name, editWS.max_row, name, name, editWS.max_row, dataUnit))
            ...
            coverageWS.cell(row=coverageWS.max_row, column=4).value = ('=COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"Passed",\'%s\'!C10:\'%s\'!C%d,"-40C",\'%s\'!B10:\'%s\'!B%d,"%s")&"/"&COUNTIFS(\'%s\'!C10:\'%s\'!C%d,"-40C",\'%s\'!B10:\'%s\'!B%d,"%s")' % (name, name, editWS.max_row, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit))
            coverageWS.cell(row=coverageWS.max_row, column=5).value = ('=COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"Passed",\'%s\'!C10:\'%s\'!C%d,"+25C",\'%s\'!B10:\'%s\'!B%d,"%s")&"/"&COUNTIFS(\'%s\'!C10:\'%s\'!C%d,"+25C",\'%s\'!B10:\'%s\'!B%d,"%s")' % (name, name, editWS.max_row, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit))
            coverageWS.cell(row=coverageWS.max_row, column=6).value = ('=COUNTIFS(\'%s\'!E10:\'%s\'!E%d,"Passed",\'%s\'!C10:\'%s\'!C%d,"+55C",\'%s\'!B10:\'%s\'!B%d,"%s")&"/"&COUNTIFS(\'%s\'!C10:\'%s\'!C%d,"+55C",\'%s\'!B10:\'%s\'!B%d,"%s")' % (name, name, editWS.max_row, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit, name, name, editWS.max_row, name, name, editWS.max_row, dataUnit))
