    ...
    def HyperLinkBuilding(i):
        ...
        if picOrg != None:
            if editWS.cell(row=i, column=imageCoordinate).value == None:
                ...
                # 儲存 Test Name 欄位
                name = (editWS.cell(row=i, column=nameCoordinate).value)
                        ...
                        for k in range(len(dir)):
                            ...
                            if unit == (dir[k][1:-1].split("]["))[0] and temp == (dir[k][1:-1].split("]["))[1]:
                                ...
                                if startTimeSec > folderTimeSec and (startTimeSec-folderTimeSec) < compare:
                                    ...
                                    finalDir = ([ name for name in os.listdir((folder[j] + "/" + dir[k])) if os.path.isdir(os.path.join((folder[j] + "/" + dir[k]), name)) ])
                                    for x in range(len(finalDir)):
                                        # 找尋放照片的資料夾
                                        if comboboxWSText.get() == finalDir[x][:-5]:
                                            picDir = os.listdir(folder[j] + "/" + dir[k] + "/" + finalDir[x])
                                            ...
                                            for y in range(len(picDir)):
                                                ...
                                                if pic == picDir[y]:
                                                    picPath[i - 10] = (folder[j] + "/" + dir[k] + "/" + finalDir[x])
                                                    picName[i - 10] = picDir[y]
                                                    sessionPath[i - 10] = dir[k]

                elif "RX_OOB_SCA" in (comboboxWSText.get()):
                    pic = picOrg + ".png"
                    for j in range(len(folder)):
                        compare = 10000000000.0
                        # 讀取所有符合工作表名稱資料夾的子資料夾
        ...
        if picPath[i - 10] != "Path":
            try:
                if picNameNew[i - 10] == "picName":
                    shutil.copy(picPath[i - 10], newImage)
                    newPath = "Image/" + newImage.split("/")[-1] + "/" + (picPath[i - 10].split("/")[-1])
                    ...
                    shutil.copyfile(os.path.join(picPath[i - 10], picName[i - 10]),os.path.join(newImage, picNameNew[i - 10] + ".png"))
