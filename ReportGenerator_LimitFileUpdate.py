...
while True:
    ...
    for executeCount in range(len(limitFilePath)):
        ...
        if True:
            ...
            if name in ExcelWS:
                print("Updating Existing Report to match Limit File\n")
                editWS = ExcelWB[name]
                tempMinus = 0
                ...
                if low != "":
                    ...
                    for LowWSname in range(1, LowWS.max_row+1):
                        Allname.append(LowWS.cell(row=LowWSname, column=7).value)
                if high != "":
                    HighWS = LimitFileWB[high]
                    for HighWSname in range(1, HighWS.max_row+1):
                        Allname.append(HighWS.cell(row=HighWSname, column=7).value)
                for editWSname in range(10, editWS.max_row+1):
                    if editWSname > NormalWS.max_row + LowWS.max_row + HighWS.max_row + 9:
                        editWS.delete_rows(NormalWS.max_row + LowWS.max_row + HighWS.max_row + 10)
                    elif editWS.cell(row=editWSname, column=4).value == "" or editWS.cell(row=editWSname, column=4).value == None:
                        editWS.delete_rows(editWSname)
                    elif editWS.cell(row=editWSname, column=4).value not in Allname:
                        editWS.delete_rows(editWSname)
                    # Progress Bar ( 掃描檔案，檢查是否有項目刪減 )
                    #global tempMinus
                    tempMinus += 1
                    print('\r' + '%.2f%% |%s%s| %d/%d' % ((float(tempMinus/totalMinus*100)), '█' * int(tempMinus*50/totalMinus), ' ' * (50-int(tempMinus*50/totalMinus)), (editWSname-9), (editWS.max_row-9)), end='')

                tempPlus = 0
                totalPlus = NormalWS.max_row + LowWS.max_row + HighWS.max_row
                print("\n\nCheck if Limit File has items added")
                print("Check normal temperature ( RT )\n")
                if normal != "":
                    for NormalWsCount in range(1, NormalWS.max_row+1):
                        ...
                        if Test_Name != editWS.cell(row=NormalWsCount+9, column=4).value:
                            editWS.insert_rows(NormalWsCount+9)
                        ...
                        elif Test_Name == editWS.cell(row=NormalWsCount+9, column=4).value and ((Temp != editWS.cell(row=NormalWsCount+9, column=3).value) or (Unit != editWS.cell(row=NormalWsCount+9, column=7).value) or (Compare != editWS.cell(row=NormalWsCount+9, column=8).value) or (L_LMT != editWS.cell(row=NormalWsCount+9, column=9).value) or (H_LMT != editWS.cell(row=NormalWsCount+9, column=10).value)):
                            editWS.cell(row=NormalWsCount+9, column=1).value = TS_No
                            editWS.cell(row=NormalWsCount+9, column=3).value = Temp
                            editWS.cell(row=NormalWsCount+9, column=7).value = Unit
                            editWS.cell(row=NormalWsCount+9, column=8).value = Compare
                        ...
                        tempPlus += 1
                        print('\r' + '%.2f%% |%s%s| %d/%d' % ((float(tempPlus/totalPlus*100)), '█' * int(tempPlus*50/totalPlus), ' ' * (50-int(tempPlus*50/totalPlus)), (NormalWsCount), (NormalWS.max_row+LowWS.max_row+HighWS.max_row)), end='')
                        Counter = NormalWsCount

                print("\n\nCheck for low temperature ( CT )\n")
                if low != "":
                    for LowWsCount in range(1, LowWS.max_row+1):
                        ...
                        if Test_Name != editWS.cell(row=Counter+LowWsCount+9, column=4).value:
                            editWS.insert_rows(Counter+LowWsCount+9)
                        ...
                        elif Test_Name == editWS.cell(row=Counter+LowWsCount+9, column=4).value and ((Temp != editWS.cell(row=Counter+LowWsCount+9, column=3).value) or (Unit != editWS.cell(row=Counter+LowWsCount+9, column=7).value) or (Compare != editWS.cell(row=Counter+LowWsCount+9, column=8).value) or (L_LMT != editWS.cell(row=Counter+LowWsCount+9, column=9).value) or (H_LMT != editWS.cell(row=Counter+LowWsCount+9, column=10).value)):
                            editWS.cell(row=Counter+LowWsCount+9, column=1).value = TS_No
                            editWS.cell(row=Counter+LowWsCount+9, column=3).value = Temp
                            editWS.cell(row=Counter+LowWsCount+9, column=7).value = Unit
                            editWS.cell(row=Counter+LowWsCount+9, column=8).value = Compare
                        ...
                        tempPlus += 1
                        print('\r' + '%.2f%% |%s%s| %d/%d' % ((float(tempPlus/totalPlus*100)), '█' * int(tempPlus*50/totalPlus), ' ' * (50-int(tempPlus*50/totalPlus)), (LowWsCount), (NormalWS.max_row+LowWS.max_row+HighWS.max_row)), end='')
                        LowCount = LowWsCount
                ...
                if high != "":
                    for HighWsCount in range(1, HighWS.max_row+1):
                        ...
                        if Test_Name != editWS.cell(row=Counter+HighWsCount+9, column=4).value:
                            editWS.insert_rows(Counter+HighWsCount+9)
                        ...
                        elif Test_Name == editWS.cell(row=Counter+HighWsCount+9, column=4).value and ((Temp != editWS.cell(row=Counter+HighWsCount+9, column=3).value) or (Unit != editWS.cell(row=Counter+HighWsCount+9, column=7).value) or (Compare != editWS.cell(row=Counter+HighWsCount+9, column=8).value) or (L_LMT != editWS.cell(row=Counter+HighWsCount+9, column=9).value) or (H_LMT != editWS.cell(row=Counter+HighWsCount+9, column=10).value)):
                            editWS.cell(row=Counter+HighWsCount+9, column=1).value = TS_No
                            editWS.cell(row=Counter+HighWsCount+9, column=3).value = Temp
                            editWS.cell(row=Counter+HighWsCount+9, column=7).value = Unit
                            editWS.cell(row=Counter+HighWsCount+9, column=8).value = Compare
                        ...
                        tempPlus += 1
                        print('\r' + '%.2f%% |%s%s| %d/%d' % ((float(tempPlus/totalPlus*100)), '█' * int(tempPlus*50/totalPlus), ' ' * (50-int(tempPlus*50/totalPlus)), (HighWsCount), (NormalWS.max_row+LowWS.max_row+HighWS.max_row)), end='')
